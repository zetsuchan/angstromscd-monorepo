...
import { z } from 'zod';
import { processVOEAlert, scheduleMedicationReminder, notifyLabResults, getQueueService } from '../services/queue-service';
...
queueRoutes.post('/voe-alert', async (c) => {
  try {
    ...
    const msgId = await processVOEAlert(data.patientId, data.riskLevel, data.details);
