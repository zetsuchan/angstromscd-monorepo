...
export class VisualizationDetector {
	...
	detectVisualization(prompt: string): VisualizationRequest {
		...
		const hasMedicalVisualizationKeyword =
			this.medicalVisualizationKeywords.some((keyword) =>
				lowerPrompt.includes(keyword),
			);
		...
		if (
			...
			!hasMedicalVisualizationKeyword &&
			!hasDataAnalysisKeyword
		) {
		...
		const suggestedLibraries = this.suggestLibraries(
			visualizationType,
			lowerPrompt,
		);
		const pythonCodeTemplate = this.generateCodeTemplate(
			visualizationType,
			lowerPrompt,
		);
	...
	private determineVisualizationType(
		prompt: string,
	): "chart" | "graph" | "plot" | "table" | "heatmap" | "other" {
		...
		if (prompt.includes("heatmap") || prompt.includes("matrix"))
			return "heatmap";
		return "other";
	}
...
`;
		...
		if (
			...
			prompt.toLowerCase().includes("clinical")
		) {
			template += `
