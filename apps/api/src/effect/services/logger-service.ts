...
export class LoggerService extends Context.Tag("LoggerService")<
  LoggerService,
  {
    readonly log: (message: string, metadata?: Record<string, unknown>) => Effect.Effect<void>;
...
export const LoggerServiceProduction = Layer.succeed(LoggerService, {
  log: (message: string, metadata?: Record<string, unknown>) =>
    Effect.log(message, metadata || {}).pipe(Logger.withMinimumLogLevel(LogLevel.Info)),
