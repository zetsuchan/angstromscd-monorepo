 ...
 */
export function useEffectForRoute(
	route: keyof FeatureFlags["EFFECT_ROUTES"],
): boolean {
 ...
 */
export function useAISDKFeature(
	feature: keyof FeatureFlags["AI_SDK_FEATURES"],
): boolean {
...
export function useConvexFeature(
	feature: keyof FeatureFlags["CONVEX_FEATURES"],
): boolean {
...
export function getEnabledFeatures(): string[] {
	...
	// Route-specific
	for (const [route, enabled_flag] of Object.entries(FEATURES.EFFECT_ROUTES)) {
		if (enabled_flag) enabled.push(`Effect: ${route}`);
	}

	// AI SDK features
	for (const [feature, enabled_flag] of Object.entries(
		FEATURES.AI_SDK_FEATURES,
	)) {
