        ...
        {threads.map((thread) => (
          <button
            ...
            className={`w-8 h-8 mb-2 rounded-full flex items-center justify-center ${
              thread.isActive ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
            }`}
  ...
  return (
    <div className="w-64 bg-gray-100 border-r border-gray-200 flex flex-col h-full">
      ...
      <div className="border-t border-gray-200 p-4">
        <h3 className="font-medium text-sm text-gray-700 mb-2">Recent Alerts</h3>
        <ul>
          {alerts.map((alert) => (
            <li key={alert.id} className="mb-2">
              <button
                className={`w-full text-left py-1 px-2 rounded ${
                  alert.isRead ? 'text-gray-500' : 
                  alert.type === 'warning' ? 'text-red-600 bg-red-50' : 'text-blue-600 bg-blue-50'
                }`}
