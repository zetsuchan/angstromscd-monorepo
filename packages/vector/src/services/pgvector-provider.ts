...
import { VectorProvider, VectorProviderConfig, QueryResult, CollectionInfo } from '../interfaces/vector-provider.interface'
...
export class PgVectorProvider implements VectorProvider {
  ...
  async deleteCollection(name: string): Promise<void> {
    ...
    try {
      await client.query('DELETE FROM vector_collections WHERE name = $1', [name])
    } finally {
      client.release()
    }
  }
  ...
  async addDocuments(options: {
    ...
    try {
      ...
      const collectionId = await this.getOrCreateCollectionId(client, options.collection)
  ...
  async query(options: {
    ...
    try {
      // Get collection ID
      const collectionId = await this.getCollectionId(client, options.collection)
      if (!collectionId) {
        return {
          ids: [[]],
          documents: [[]],
  ...
  private async getCollectionId(client: PoolClient, name: string): Promise<number | null> {
    const result = await client.query(
  ...
  private async getOrCreateCollectionId(client: PoolClient, name: string): Promise<number> {
