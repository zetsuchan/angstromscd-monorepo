...
export class PGMQClient {
	...
	async read<T>(queueName: string, vt = 30, qty = 1): Promise<Message<T>[]> {
		const result = await this.sql`
...
export class MedicalQueueService extends PGMQClient {
	...
	async readVOEAlerts(limit = 10): Promise<Message<any>[]> {
		return this.read("voe_alerts", 30, limit);
	}

	async readMedicationReminders(limit = 10): Promise<Message<any>[]> {
		return this.read("medication_reminders", 30, limit);
	}

	async readLabResultNotifications(limit = 10): Promise<Message<any>[]> {
