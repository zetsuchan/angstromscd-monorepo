...
} from "../apps/api/src/types/e2b";
import {
	MedicalContext,
	type PatientInfo,
} from "../packages/context/src/medical-context";
...
export async function createDemoSession(
	...
	async function executeCode(
		input: Parameters<typeof e2bSession.executeCode>[0],
	) {
	...
	async function searchLiterature(query: string, topK = 5) {
		const url = `https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=pubmed&retmode=json&retmax=${topK}&term=${encodeURIComponent(
			query,
		)}`;
...
if (import.meta.main) {
	...
	if (!runDetailedDemo) {
		console.log(
			"Demo session initialised. Use --demo to run the walkthrough or import this module in a REPL.",
		);
		process.exit(0);
	}
	...
	const codeResult = await session.executeCode({
		code: "print(2+2)",
		language: "python",
		timeout: 30,
	});
	...
	const pubmedIds = await session.searchLiterature(
		"sickle cell vaso-occlusive event biomarkers",
		3,
	);
